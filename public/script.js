...
function _addTable() {
  ...
  $('input[data-tableid]').each(function () {
    tableID = $(this).data('tableid') > tableID ? $(this).data('tableid') : tableID;
  });
  tableID++;
  ...
  let tableTemplate = `
    <div class="table" data-tableid="${tableID}">
      ...
      <div class="row">
        <input data-tableid="${tableID}" type="text" placeholder="My Data (${tableID + 1})"/>
...
async function _askForFields(tableID) {
  ...
  function displayFields(fields, spacing, parent) {
    for (let field in fields) {
      ...
      output += `<div class='field' onclick='${showCheck ? '_toggleCheck(this)' : '_toggleChildCheck(this)'}' data-checked='false' style="padding-left:${spacing}px;" data-tableid='${tableID}' data-visible='${showCheck}' data-field='${(parent === '' ? '' : parent + '.') + field}'>
        ${showCheck ? '<div class="check"></div>' : '<div class="check nested"></div>'}
        <div class="fieldText" >
          ${field}
        </div>
      </div>`;
      if (fields[field]) {
        displayFields(fields[field], spacing + 10, (parent === '' ? '' : parent + '.') + field);
      }
    }
  }
...
function _toggleChildCheck(e) {
  ...
  let children = $('#tables').find(`[data-tableID='${parentTableID}'][data-field^='${parentField}'][data-field!='${parentField}']`);
